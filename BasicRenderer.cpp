...
void BasicRenderer::CreateFilledBox(uint32_t xPos, uint32_t yPos, uint32_t w, uint32_t h, uint32_t Colour)
{
    ...
    for(y = yPos; y <= height; y++)
    {
        ...
        {
            *(uint32_t*)(x + (y * TargetFramebuffer->PixelsPerScanline * BBP) + (void**)TargetFramebuffer->BaseAddress) = Colour;
        }
    }
}
...
void BasicRenderer::Clear(){
    ...
    int boxW = (TargetFramebuffer->Width / 4);
    int boxH = 3;
    int boxX = 0;
    int boxY = (TargetFramebuffer->Height / 10) + (26 - boxH);
    int boxC = 0xff333333;

    //CreateFilledBox(boxX, boxY, boxW, boxH, boxC);
...
void BasicRenderer::Print(const char* str)
{
    ...
    while(*chr != 0){
        ...
        {
            CursorPosition.Y = TargetFramebuffer->Height - 32;
        }
        chr++;
    }
}
...
void BasicRenderer::PutChar(char chr)
{
    if (chr == '\n' || '¦'){
        ...
        if (CursorPosition.Y > (TargetFramebuffer->Height - 32)){
